...
원하는 화질로 영상을, MP3로 음원을, 자막이 없는 영상은 음성 인식으로 대본까지 — 레퍼런스 영상은 대본을 뽑아 분석까지. macOS · Windows.
...
- 🔎 **콘텐츠 분석** — 영상 대본을 뽑아 훅·구성 흐름·CTA·핵심 메시지를 항목별로 분해 (내 AI 키로 동작, 대본·결과는 기기에만 저장)
- 🌐 **크롬 확장 연동** — 영상 페이지에서 **'다운로드'·'분석'** 버튼으로 앱에 바로 전달
...
| **Instagram** | 게시물(피드) · 릴스 | 캐러셀 형태의 이미지/영상은 다운로드 불가 |
| **TikTok** | 영상 · 카로셀 |  |
...
> ⚠️ 이 앱은 Apple 유료 개발자 서명을 붙이지 않은 개인 개발 앱이라 위 경고가 표시됩니다. 악성 프로그램이어서가 아니며, 서명 비용 없이 배포하기 위한 것입니다.
...
레퍼런스로 삼고 싶은 영상을 구조 단위로 분해해 봅니다.

1. 상단에서 **분석** 모드로 전환하고 영상 링크를 넣습니다.
2. 앱이 대본을 확보한 뒤(자막이 없으면 음성 인식), 훅·구성 흐름·CTA·핵심 메시지 등을 항목별로 정리해 줍니다.
3. 필요하면 장면이 바뀌는 지점의 이미지를 함께 뽑아 스토리보드처럼 볼 수 있습니다(설정에서 켜기).

> 분석은 **직접 등록한 AI 키(Claude · ChatGPT · Gemini)** 로 동작합니다. 설정 → **AI 분석** 탭에서 키를 넣으면 되며, 키 발급 방법도 함께 안내합니다. 키는 기기에 암호화되어 저장되고, 대본·분석 결과는 전부 내 컴퓨터에만 남습니다.

### 🧩 크롬 확장으로 더 빠르게

브라우저에서 보던 영상을 바로 앱으로 보낼 수 있습니다.

1. [`omni-chrome-extension.zip`](https://github.com/thoven-jun/omni-video-downloader-releases/releases/download/chrome-extension-v1.2/omni-chrome-extension.zip) 다운로드 후 압축 해제 ([확장 릴리스 페이지](https://github.com/thoven-jun/omni-video-downloader-releases/releases/tag/chrome-extension-v1.2))
2. Chrome 주소창에 `chrome://extensions` 입력
3. 우측 상단 **개발자 모드** 켜기
4. **압축 해제된 확장 프로그램을 로드합니다** → 압축 푼 폴더 선택
5. 이제 YouTube · Instagram · TikTok 영상 화면에 **다운로드**·**분석** 버튼이 생깁니다.
   '다운로드'는 저장을, '분석'은 콘텐츠 분석을 앱에서 바로 시작합니다.

### 📝 자막·대본 추출

- 자막 트랙이 있는 영상은 **SRT 자막 파일과 TXT 텍스트 파일**로 저장됩니다.
- 자막이 없는 영상(인스타 릴스, 틱톡 등)은 **AI 음성 인식(Whisper)** 으로 대본을 만들어 `_script.srt`, `_script.txt`로 저장합니다.
