...
- 🎞️ **장면 이미지 추출** — 장면(컷)이 바뀌는 순간을 이미지로 저장. 영상과 함께, 또는 이미지(zip)만. 장면 전환이 없는 영상은 첫 장면 한 장.
...
- 🧩 **확장 프로그램 설정 탭** — 앱 설정에서 확장 프로그램 다운로드와 설치 방법 안내
...
1. `Omni-Video-Downloader-x.x.x-universal.dmg` 다운로드 (Apple Silicon · Intel 겸용)
...
2. 영상 정보를 불러오면 **다운로드 설정** 창에서 선택:
   ...
   - **함께 받기** — 썸네일 · 자막·대본
   - **단독 추출** — 영상 없이 썸네일만 · 자막·대본만 · **샷 추출(zip)**
3. **다운로드** 클릭 → 완료

> 영상과 장면 이미지를 한 번에 받으려면 **설정 → 다운로드 → 함께 받기**에서 **'영상과 함께 샷(장면) 추출'** 을 켜 두세요. 영상 저장 후 `<제목>_shots.zip`이 같이 저장됩니다.
